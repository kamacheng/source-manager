...
export default function ResourceManager() {
    ...
    const isSubCategorySelected = () => {
        if (filterMediaType === 'all' || filterCategory === 'all') {
            return false;
        }
        // 检查当前选中的分类是否是子分类（即是否有父分类）
        return !!getParentCategoryId(filterMediaType, filterCategory);
    };
    ...
    const getCategoryColor = (mediaType, categoryId) => {
        ...
        // 颜色方案数组
        const colorSchemes = [
            'bg-blue-500/80 text-white',      // 蓝色
            'bg-purple-500/80 text-white',    // 紫色
            'bg-cyan-500/80 text-white',      // 青色
            'bg-teal-500/80 text-white',      // 蓝绿色
            'bg-emerald-500/80 text-white',   // 翠绿色
        ];
    ...
    const handleDeleteSubCategory = (type, parentId, subCategoryId) => {
        ...
        if (affectedItems.length > 0) {
            ...
            setMigrationData({
                ...
                parentId: parentId,
                isSubCategory: true,
                allowCrossCategoryMigration: true  // 标记允许跨一级分类迁移
            });
            setIsMigrationModalOpen(true);
            setSelectedMigrationTarget(null);
        } else {
            // 没有资源在使用，检查是否是最后一个子分类
            if (remainingChildren.length === 0) {
                showNotification('无法删除：每个业务分类必须至少保留一个子分类。如需删除此子分类，请先删除其业务分类（确保该业务分类下无子分类）。', 'error');
                return;
            }
    ...
    const handleDeleteCategory = (type, id) => {
        // 检查该业务分类是否有子分类
        const category = categoryConfig[type]?.find(c => c.id === id);
        const hasChildren = category?.children && category.children.length > 0;

        if (hasChildren) {
            showNotification(`无法删除：该业务分类下还有 ${category.children.length} 个子分类，请先删除所有子分类。`, 'error');
            return;
        }

        // 如果没有子分类，可以直接删除（需要确认）
        if (window.confirm(`确定要删除业务分类 "${category?.label}"吗？`)) {
            setCategoryConfig(prev => ({
                ...prev,
                [type]: prev[type].filter(c => c.id !== id)
            }));
            showNotification('业务分类删除成功');
        }
    };
                        ...
                        <div className="bg-white rounded-lg border border-slate-200 shadow-sm p-4 md:p-5 mb-6">
                            <div className="flex flex-col md:flex-row items-center gap-3">
                                ...
                                <button
                                    ...
                                    title={isSubCategorySelected() ? '批量上传资源' : '请先选择具体子类'}
                                >
                                    <Upload size={18} />
                                    <span>批量上传</span>
                                </button>
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {categoryConfig[filterMediaType].map(cat => (
                                        <div key={cat.id} className="group relative">
                                            <button
                                                className={`w-full px-3 py-1.5 rounded-lg text-sm font-medium transition-all flex items-center justify-between gap-1.5 ${filterCategory === cat.id
                                            ...
                                            >
                                                ...
                                                {cat.children && cat.children.length > 0 && (
                                                    <div
                                                        ...
                                                        className="cursor-pointer text-xs opacity-70 hover:opacity-100 transition-opacity"
                                                        title={expandedCategories[cat.id] ? '收起子分类' : '展开子分类'}
                                                    >
                                            ...
                                            {cat.children && cat.children.length > 0 && expandedCategories[cat.id] && (
                                                <div className="absolute left-0 mt-1 bg-white border border-slate-200 rounded-lg shadow-lg z-50 w-40">
                                    ...
                                    <div key={item.id} className="group bg-white rounded-lg border border-slate-200 overflow-hidden hover:shadow-lg hover:border-indigo-300 transition-all duration-300 relative flex h-32">
                                        ...
                                        <div className="flex-1 p-3 flex flex-col">
                                            ...
                                            <div className="mb-2 w-fit">
                                                <span className={`px-2 py-1 text-[10px] uppercase font-bold tracking-wide rounded-md flex items-center gap-1 ${getCategoryColor(item.mediaType, item.category)}`}>
                                                                ...
                                                                <td className="px-4 py-2 text-right space-x-2 flex justify-end">
                                                                    ...
                                                                    {cat.id !== 'all' && cat.id !== 'image' && cat.id !== 'video' && cat.id !== 'audio' && (
                                                                        <button
                                                                            ...
                                                                            className={`transition-colors ${cat.children && cat.children.length > 0
                                                                                ? 'text-slate-200 cursor-not-allowed opacity-50'
                                                                                : 'text-slate-300 hover:text-red-500'
                                                                                }`}
                                                                            title={cat.children && cat.children.length > 0 ? `无法删除：该分类下有 ${cat.children.length} 个子分类` : '删除业务分类'}
                                                                        >
                                                                            <MinusCircle size={16} />
                                                                        </button>
                                                                    )}
                        ...
                        <div className="p-6 space-y-4">
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="space-y-2 max-h-48 overflow-y-auto">
                                    {(() => {
                                        // 子分类迁移：如果允许跨一级分类迁移，显示所有子分类；否则只显示同级
                                        if (migrationData.isSubCategory && migrationData.parentId) {
                                            if (migrationData.allowCrossCategoryMigration) {
                                                // 显示所有其他子分类（包括跨一级分类）
                                                const allSubCategories = [];
                                                categoryConfig[migrationData.type]?.forEach(cat => {
                                                    if (cat.children && cat.children.length > 0) {
                                                        cat.children.forEach(child => {
                                                            // 排除要删除的子分类本身
                                                            if (child.id !== migrationData.categoryId) {
